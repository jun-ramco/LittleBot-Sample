/**
 * Created by Wang Jun on 2/2/2017.
 */
...
var bot = new builder.UniversalBot(connector, [
    ...
    function (session, results) {
        ...
        var msg = new builder.Message(session)
            .attachmentLayout(builder.AttachmentLayout.carousel).addAttachment(card);
        session.send(msg);
    },
    function (session, results) {
    if (results.response == 'profile')
...
function createHeroCard(session) {
    return new builder.HeroCard(session)
        ...
        .buttons([
            ...
            builder.CardAction.imBack(session, 'profile', 'imback?'),
            builder.CardAction.postBack(session, 'profile', 'Postback?'),
            builder.CardAction.dialogAction(session, 'profile', 'dialogAction?')
        ]);
}
